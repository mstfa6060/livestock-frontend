...
export const alt = "Seller - Livestock Trading";
...
async function fetchSeller(id: string) {
  try {
    const res = await fetch(`${API_BASE}/livestocktrading/Sellers/Detail`, {
      method: "POST",
...
export default async function Image({
  ...
  const description = seller.description
    ? seller.description.length > 120 ? seller.description.slice(0, 117) + "..." : seller.description
    : null;
  const logoUrl = seller.logoUrl || null;
  const rating = seller.averageRating ? Number(seller.averageRating).toFixed(1) : null;
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Left accent bar */}
        <div style={{ width: 8, height: "100%", background: "#16a34a", display: "flex" }} />
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 28, marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                {seller.isVerified && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 6, fontSize: 18, color: "#16a34a", fontWeight: 600 }}>
                    <span>✅</span>
            ...
            <div style={{ display: "flex", gap: 40, marginTop: 32 }}>
              ...
              {totalSales > 0 && (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <span style={{ fontSize: 32, fontWeight: 800, color: "#0f172a" }}>{totalSales}</span>
                  <span style={{ fontSize: 15, color: "#94a3b8", marginTop: 4 }}>satış</span>
                </div>
              )}
            </div>
          </div>
